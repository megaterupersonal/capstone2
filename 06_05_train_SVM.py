# 06_05_train_SVM.py
# Support Vector Machine (RBF) with exact top-20% workload policy
...
#   (rank-based top-K using decision_function scores; not a probability threshold)
# - Saves: model, metrics, predictions, CV summary, best params, policy details
...
OUT_DIR = os.path.join(FEAT_DIR, "models_svm")
pathlib.Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

MODEL_PATH      = os.path.join(OUT_DIR, "svm_best_model.joblib")
METRICS_PATH    = os.path.join(OUT_DIR, "svm_test_metrics.json")
PRED_PATH       = os.path.join(OUT_DIR, "svm_test_predictions.csv")
CVRESULTS_PATH  = os.path.join(OUT_DIR, "svm_cv_results.csv")
BESTPARAMS_PATH = os.path.join(OUT_DIR, "svm_best_params.json")
POLICY_PATH     = os.path.join(OUT_DIR, "svm_decision_policy.json")
PRVAL_PATH      = os.path.join(OUT_DIR, "svm_val_pr_curve.csv")
...
# ---------- SVM base model & search space ----------
svc_base = SVC(
    kernel="rbf",
    class_weight="balanced",
    probability=False,          # we use decision_function for ranking
    random_state=RANDOM_STATE,
)
...
search = RandomizedSearchCV(
    ...
    n_iter=12,                    # modest search; SVM is heavier than LR
...
print("[FIT] SVM (RBF) RandomizedSearchCV (PR-AUC) starting…")
search.fit(X_train, y_train)
best_svm = search.best_estimator_
print(f"[FIT] Best params: {search.best_params_}")
print(f"[FIT] Best CV (PR-AUC): {search.best_score_:.4f}")

# Defensive final refit on Train_in (not strictly needed, but consistent)
best_svm.fit(X_train, y_train)

# ---------- Helper: rank-based top-K predictions ----------
def topk_pred_from_scores(y_score: np.ndarray, frac: float):
...
#are labeled 1, and the rest 0. Also returns the score at the K-th position.
    ...
    order = np.argsort(y_score)[::-1]  # descending by score
    topk_idx = order[:k]
...
val_scores = best_svm.decision_function(X_val)  # margins as risk scores
val_pred, val_kth_score, val_k = topk_pred_from_scores(val_scores, WORKLOAD_FRAC)
...
# Save PR curve diagnostics on VAL (using decision_function scores)
prec_curve, rec_curve, thr_curve = precision_recall_curve(y_val, val_scores)
rows = []
...
pred_test, test_kth_score, test_k = topk_pred_from_scores(test_scores, WORKLOAD_FRAC)
...
pred_df = pl.DataFrame(
    {
        ...
        "y_true": y_test,
        "y_pred": pred_test,
        "y_score": test_scores,   # decision_function scores
    }
)
