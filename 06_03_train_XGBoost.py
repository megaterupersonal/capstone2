# 06_03_train_XGBoost.py  (XGBoost with exact top-20% workload policy)
...
MODEL_PATH       = os.path.join(OUT_DIR, "xgb_best_model.joblib")
METRICS_PATH     = os.path.join(OUT_DIR, "xgb_test_metrics.json")
PRED_PATH        = os.path.join(OUT_DIR, "xgb_test_predictions.csv")
FEATIMP_PATH     = os.path.join(OUT_DIR, "xgb_feature_importances.csv")
CVRESULTS_PATH   = os.path.join(OUT_DIR, "xgb_cv_results.csv")
BESTPARAMS_PATH  = os.path.join(OUT_DIR, "xgb_best_params.json")
POLICY_PATH      = os.path.join(OUT_DIR, "xgb_decision_policy.json")
PRVAL_PATH       = os.path.join(OUT_DIR, "xgb_val_pr_curve.csv")  # optional diagnostic
...
search = RandomizedSearchCV(
    ...
    n_iter=40,                    # tweak down if runtime is too long
...
# XGBoost feature importance (importance_type='gain' by default is usually more meaningful)
