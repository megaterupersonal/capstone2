# 06_06_train_KNN.py
# K-Nearest Neighbours with exact top-20% workload policy (linear features)
...
#   (rank-based top-K using predict_proba[:, 1])
...
OUT_DIR = os.path.join(FEAT_DIR, "models_knn")
pathlib.Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

MODEL_PATH      = os.path.join(OUT_DIR, "knn_best_model.joblib")
METRICS_PATH    = os.path.join(OUT_DIR, "knn_test_metrics.json")
PRED_PATH       = os.path.join(OUT_DIR, "knn_test_predictions.csv")
CVRESULTS_PATH  = os.path.join(OUT_DIR, "knn_cv_results.csv")
BESTPARAMS_PATH = os.path.join(OUT_DIR, "knn_best_params.json")
POLICY_PATH     = os.path.join(OUT_DIR, "knn_decision_policy.json")
PRVAL_PATH      = os.path.join(OUT_DIR, "knn_val_pr_curve.csv")
...
# ---------- KNN base model & search space ----------
# Note: features are already standardized in 05_09b for linear models.
knn_base = KNeighborsClassifier()

# Reasonable search space:
# - n_neighbors: small-to-moderate values to balance bias/variance
# - weights: uniform vs distance
# - metric: Minkowski with p=1 (Manhattan) or p=2 (Euclidean)
# - leaf_size: tree search efficiency tuning
param_dist = {
    "n_neighbors": [5, 10, 15, 25, 35, 45],
...
search = RandomizedSearchCV(
    ...
    n_iter=25,                    # KNN is reasonably fast, explore a bit
...
print("[FIT] KNN RandomizedSearchCV (PR-AUC) starting…")
search.fit(X_train, y_train)
best_knn = search.best_estimator_
print(f"[FIT] Best params: {search.best_params_}")
print(f"[FIT] Best CV (PR-AUC): {search.best_score_:.4f}")

# KNN is lazy, no further refit needed beyond search.refit
...
proba_val = best_knn.predict_proba(X_val)[:, 1]
