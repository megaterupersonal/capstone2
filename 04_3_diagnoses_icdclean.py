# 04_3_diagnoses_icdclean.py
# This file cleans and preprocesses the DIAGNOSES_ICD_sampled.parquet table
...
# ---- Load ----
diag_before = pl.read_parquet(os.path.join(PARQUET_DIR, "DIAGNOSES_ICD_sampled.parquet"))
diag = diag_before.clone()
print(f"Loaded DIAGNOSES_ICD: {diag.shape[0]:,} rows, {diag.shape[1]} columns")

# ---- Cast datatypes ----
diag = diag.with_columns([
    pl.col("SUBJECT_ID").cast(pl.Int64),
    pl.col("HADM_ID").cast(pl.Int64),
    pl.col("SEQ_NUM").cast(pl.Int64),
    pl.col("ICD9_CODE").cast(pl.Utf8),
    pl.col("SHORT_TITLE").cast(pl.Utf8),
    pl.col("LONG_TITLE").cast(pl.Utf8),
])

# ---- Drop null/empty ICD codes ----
before_count = diag.shape[0]
diag = diag.filter(
    (pl.col("ICD9_CODE").is_not_null()) &
    (pl.col("ICD9_CODE").str.strip_chars() != "")
)
removed = before_count - diag.shape[0]
print(f"Removed {removed:,} rows with null/empty ICD9_CODE.")

# ---- Standardize ICD codes ----
diag = diag.with_columns([
    pl.col("ICD9_CODE").str.strip_chars().str.to_uppercase().alias("ICD9_CODE")
])

# ---- Remove duplicates ----
before_count = diag.shape[0]
diag = diag.unique(subset=["SUBJECT_ID", "HADM_ID", "ICD9_CODE"])
print(f"Removed {before_count - diag.shape[0]:,} duplicate rows.")

# ---- Clean textual fields ----
diag = diag.with_columns([
    pl.col("SHORT_TITLE").str.strip_chars().str.to_titlecase().alias("SHORT_TITLE"),
    pl.col("LONG_TITLE").str.strip_chars().str.to_titlecase().alias("LONG_TITLE"),
])

# ---- Derive 3-digit ICD9 group ----
# Some codes include decimals or letters; keep only leading numeric or alpha prefix
diag = diag.with_columns([
    pl.col("ICD9_CODE").str.extract(r"^([A-Z0-9]{3})").alias("ICD9_GROUP")
])

# ---- Profiling summary ----
prof_before = profile_dataframe(diag_before, table_name="DIAGNOSES_ICD_sampled")
prof_after  = profile_dataframe(diag,        table_name="DIAGNOSES_ICD_cleaned")
comparison  = compare_profiles(prof_before, prof_after)

summary_path = os.path.join(OUTPUT_DIR, "DIAGNOSES_ICD_cleaning_summary.csv")
comparison.write_csv(summary_path)
print(f"[REPORT] Wrote before/after summary → {summary_path}")

# ---- Save cleaned file ----
out_path = os.path.join(OUTPUT_DIR, "DIAGNOSES_ICD_cleaned.parquet")
diag.write_parquet(out_path, compression="zstd")
print(f"Saved cleaned DIAGNOSES_ICD → {out_path}")
print("Done.")
