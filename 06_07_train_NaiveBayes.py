# 06_07_train_NaiveBayes.py
# Gaussian Naive Bayes with exact top-20% workload policy (linear features)
...
# - Saves: model, metrics, predictions, CV summary, best params, policy details, val PR curve
...
OUT_DIR = os.path.join(FEAT_DIR, "models_nb")
pathlib.Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

MODEL_PATH      = os.path.join(OUT_DIR, "nb_best_model.joblib")
METRICS_PATH    = os.path.join(OUT_DIR, "nb_test_metrics.json")
PRED_PATH       = os.path.join(OUT_DIR, "nb_test_predictions.csv")
CVRESULTS_PATH  = os.path.join(OUT_DIR, "nb_cv_results.csv")
BESTPARAMS_PATH = os.path.join(OUT_DIR, "nb_best_params.json")
POLICY_PATH     = os.path.join(OUT_DIR, "nb_decision_policy.json")
PRVAL_PATH      = os.path.join(OUT_DIR, "nb_val_pr_curve.csv")
...
# ---------- Naive Bayes base model & search space ----------
# Tune var_smoothing, which controls a small variance added for numerical stability.
nb_base = GaussianNB()

param_dist = {
    # explore several orders of magnitude; RandomizedSearch will sample from this grid
...
search = RandomizedSearchCV(
    ...
    n_iter=15,                   # small but reasonable; can bump to 20+ if desired
...
print("[FIT] NaiveBayes RandomizedSearchCV (PR-AUC) starting…")
...
# Defensive final refit on Train_in (not strictly necessary, but consistent)
...
proba_val = best_nb.predict_proba(X_val)[:, 1]
...
proba_test = best_nb.predict_proba(X_test)[:, 1]
