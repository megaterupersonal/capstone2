# 06_02_train_DecisionTree.py  (Decision Tree with exact top-20% workload policy)
# - Custom split: Train/Valid/Test with test prevalence ≈ 5.52% positives (realistic base rate)
# - RandomizedSearchCV scored by PR-AUC (average_precision)
# - Operating policy: flag EXACTLY the top 20% highest-risk patients on each set
#   (rank-based top-K, not value-based percentile)
# - Saves: model, metrics, predictions, feature importances, CV summary, best params, chosen K and threshold
...
OUT_DIR  = os.path.join(FEAT_DIR, "models_decision_tree")
pathlib.Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

MODEL_PATH       = os.path.join(OUT_DIR, "dt_best_model.joblib")
METRICS_PATH     = os.path.join(OUT_DIR, "dt_test_metrics.json")
PRED_PATH        = os.path.join(OUT_DIR, "dt_test_predictions.csv")
FEATIMP_PATH     = os.path.join(OUT_DIR, "dt_feature_importances.csv")
CVRESULTS_PATH   = os.path.join(OUT_DIR, "dt_cv_results.csv")
BESTPARAMS_PATH  = os.path.join(OUT_DIR, "dt_best_params.json")
THRESHOLD_PATH   = os.path.join(OUT_DIR, "dt_decision_threshold.json")
VAL_TABLE_PATH   = os.path.join(OUT_DIR, "dt_val_threshold_table.csv")  # diagnostics
...
print("[FIT] DecisionTree RandomizedSearchCV (PR-AUC) starting…")
...
# ---------- Threshold / policy selection on VALIDATION (rank-based top-20%) ----------
val_proba = best_dt.predict_proba(X_val)[:, 1]
val_pred, val_kth_score, val_k = topk_pred(val_proba, WORKLOAD_FRAC)
...
print(f"[POLICY] Selected policy: {policy}")
print("[VAL] Classification report @top-20% workload:")
print(classification_report(y_val, val_pred, digits=3))

# Save threshold table (PR curve + F2 across probability thresholds; independent of top-K policy)
prec_curve, rec_curve, thr_curve = precision_recall_curve(y_val, val_proba)
rows = []
