...
# ---- Load ----
proc_before = pl.read_parquet(os.path.join(PARQUET_DIR, "PROCEDURES_ICD_sampled.parquet"))
proc = proc_before.clone()
print(f"Loaded PROCEDURES_ICD: {proc.shape[0]:,} rows, {proc.shape[1]} columns")
...
# ---- Drop null/empty ICD9 codes ----
before = proc.height
proc = proc.filter(
    (pl.col("ICD9_CODE").is_not_null()) &
    (pl.col("ICD9_CODE").str.strip_chars() != "")
)
print(f"Removed {before - proc.height:,} rows with null/empty ICD9_CODE.")
...
# ---- Remove duplicates ----
before = proc.height
proc = proc.unique(subset=["SUBJECT_ID", "HADM_ID", "ICD9_CODE"])
print(f"Removed {before - proc.height:,} duplicate rows.")
...
# ---- Profiling summary ----
prof_before = profile_dataframe(proc_before, table_name="PROCEDURES_ICD_sampled")
prof_after  = profile_dataframe(proc,        table_name="PROCEDURES_ICD_cleaned")
comparison  = compare_profiles(prof_before, prof_after)

summary_path = os.path.join(OUTPUT_DIR, "PROCEDURES_ICD_cleaning_summary.csv")
comparison.write_csv(summary_path)
print(f"[REPORT] Wrote before/after summary → {summary_path}")

# ---- Save cleaned file ----
out_path = os.path.join(OUTPUT_DIR, "PROCEDURES_ICD_cleaned.parquet")
proc.write_parquet(out_path, compression="zstd")
print(f"Saved cleaned PROCEDURES_ICD → {out_path}")
print("Done.")
