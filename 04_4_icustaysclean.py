# 04_4_icustaysclean.py
# This file cleans and preprocesses the ICUSTAYS_sampled.parquet table.
...
# ---- Load ----
icu_before = pl.read_parquet(os.path.join(PARQUET_DIR, "ICUSTAYS_sampled.parquet"))
icu = icu_before.clone()
print(f"Loaded ICUSTAYS: {icu.shape[0]:,} rows, {icu.shape[1]} columns")
...
icu = icu.with_columns([
    ...
    pl.col("LOS").cast(pl.Float64)
])

# ---- Drop duplicates ----
icu = icu.unique(subset=["ICUSTAY_ID"])
print(f"After dropping duplicates: {icu.shape[0]:,}")

# ---- Remove invalid or negative LOS ----
initial_count = icu.shape[0]
icu = icu.filter(
    (pl.col("LOS").is_not_null()) &
    (pl.col("LOS") >= 0) &
    (pl.col("OUTTIME") > pl.col("INTIME"))
)
removed = initial_count - icu.shape[0]
print(f"Removed {removed:,} rows with invalid LOS or timestamps.")

# ---- Round LOS to 2 dp ----
icu = icu.with_columns([
    pl.col("LOS").round(2).alias("LOS")
])

# ---- Optional: recompute LOS to double-check ----
# icu = icu.with_columns([
#     ((pl.col("OUTTIME") - pl.col("INTIME")).dt.total_days()).round(2).alias("LOS_RECALC")
# ])

# ---- Profiling summary ----
prof_before = profile_dataframe(icu_before, table_name="ICUSTAYS_sampled")
prof_after  = profile_dataframe(icu,        table_name="ICUSTAYS_cleaned")
comparison  = compare_profiles(prof_before, prof_after)

summary_path = os.path.join(OUTPUT_DIR, "ICUSTAYS_cleaning_summary.csv")
comparison.write_csv(summary_path)
print(f"[REPORT] Wrote before/after summary → {summary_path}")

icu = icu.with_columns([
    pl.lit(1).alias("IS_ICU_STAY")
])


# ---- Save cleaned file ----
out_path = os.path.join(OUTPUT_DIR, "ICUSTAYS_cleaned.parquet")
icu.write_parquet(out_path, compression="zstd")
print(f"Saved cleaned ICUSTAYS → {out_path}")
print("Done.")
