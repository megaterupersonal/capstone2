# 06_04_train_LogisticRegression.py
# Logistic Regression with exact top-20% workload policy (linear features)
# - Uses final_linear_features.parquet / final_linear_labels.csv
...
#   (rank-based top-K; not a probability threshold)
# - Saves: model, metrics, predictions, CV summary, best params, policy details, coef table
...
# Linear-preprocessed features/labels
X_PATH = os.path.join(FEAT_DIR, "final_linear_features.parquet")
y_PATH = os.path.join(FEAT_DIR, "final_linear_labels.csv")

OUT_DIR = os.path.join(FEAT_DIR, "models_logistic")
pathlib.Path(OUT_DIR).mkdir(parents=True, exist_ok=True)

MODEL_PATH      = os.path.join(OUT_DIR, "lr_best_model.joblib")
METRICS_PATH    = os.path.join(OUT_DIR, "lr_test_metrics.json")
PRED_PATH       = os.path.join(OUT_DIR, "lr_test_predictions.csv")
CVRESULTS_PATH  = os.path.join(OUT_DIR, "lr_cv_results.csv")
BESTPARAMS_PATH = os.path.join(OUT_DIR, "lr_best_params.json")
POLICY_PATH     = os.path.join(OUT_DIR, "lr_decision_policy.json")
PRVAL_PATH      = os.path.join(OUT_DIR, "lr_val_pr_curve.csv")
COEF_PATH       = os.path.join(OUT_DIR, "lr_coefficients.csv")
...
# ---------- Logistic Regression base model & search space ----------
# Note: features are already standardized in 05_09b
lr_base = LogisticRegression(
    solver="liblinear",        # good for L2 + class_weight, handles imbalance
    penalty="l2",
    max_iter=2000,
    class_weight="balanced",   # baseline class balancing
    random_state=RANDOM_STATE,
)

# Simple but effective search over regularization strength
param_dist = {
    "C": np.logspace(-3, 2, 20),   # from 0.001 to 100
}
...
search = RandomizedSearchCV(
    ...
    n_iter=20,                    # can bump to 30–40 if you want
    scoring="average_precision",  # PR-AUC; optimises ranking quality
    n_jobs=-1,
    cv=cv,
    verbose=1,
    random_state=RANDOM_STATE,
...
print("[FIT] LogisticRegression RandomizedSearchCV (PR-AUC) starting…")
...
# Defensive final refit on Train_in with the same weights
best_lr.fit(X_train, y_train, sample_weight=sample_w)
...
# Save PR curve diagnostics on VAL (threshold-based, independent of top-K policy)
...
print(f"[SAVE] Validation PR-curve diagnostics → {PRVAL_PATH}")
...
# Coefficient table (as "feature importance" for LR)
coefs = getattr(best_lr, "coef_", None)
if coefs is not None:
    coefs = coefs[0]  # binary case: shape (1, n_features)
    coef_df = (
        pl.DataFrame(
            {
                "feature": feat_names,
